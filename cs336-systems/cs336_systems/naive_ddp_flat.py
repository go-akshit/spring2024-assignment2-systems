...
def run(args, model, optimizer, num_iterations, inputs, target):
    for _ in range(num_iterations):
        ...
        flat_grads = torch._utils._flatten_dense_tensors(grad_list)

        # All-reduce the concatenated gradient tensor
        dist.all_reduce(tensor=param.grad, op=dist.ReduceOp.AVG, async_op=False)

        # Unflatten the gradients back to the original shapes and assign them to the parameters
        grad_tensors = torch._utils._unflatten_dense_tensors(flat_grads, grad_list)
        for param, grad in zip([param for param in model.parameters() if param.grad is not None], grad_tensors):
...
def ddp(args):
    ...
    print(f"sharded_input {rank} = {sharded_input}")
    print(f"{sharded_input.shape}")
    print(f"sharded_target {rank} = {sharded_target}")
